...
class Docs {
  ...
  static indexing (params){
    ...
    const flush = cb => {
      ...
      if( config.indexesSort.length > 0 ){
        indexes.sort((a ,b) => {
          const sortItem = config.indexesSort.find(item => a[item.key] && b[item.key]);
          if( sortItem ){
            return sortItem.isDesc ? a[sortItem.key] < b[sortItem.key] : a[sortItem.key] > b[sortItem.key];
          }
        });
      }
...
indexing: true,
    ...
    const transform = function(file, enc, cb){
      ...
      //this.emit('error', new $.util.PluginError('gulp-diff', 'Streaming not supported'));
